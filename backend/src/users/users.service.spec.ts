...
describe('UsersService', () => {
  ...
  describe('update', () => {
    ...
    it('throws ConflictException if new mobile number is already taken by someone else', async () => {
      repo.findOne
        .mockResolvedValueOnce({ id: 'user-1', mobileNumber: '09120000000' }) // findOne(id)
        .mockResolvedValueOnce({ id: 'user-2', mobileNumber: '09123456789' }); // mobile lookup

      await expect(
        service.update('user-1', { mobileNumber: '09123456789' } as any),
      ).rejects.toThrow(ConflictException);
    });
    ...
    it('throws ConflictException if new email is already taken by someone else', async () => {
      ...
      await expect(
        service.update('user-1', { email: 'taken@b.com' }),
      ).rejects.toThrow(ConflictException);
    });
  });
});
